...
def build_input_layer(data_set):
    ...
    for i in range(len(data_set)):
        if isfloat(data_set[i, 6]):
            ...
            input_data_set[i, 2] = list_of_main_categorie.index(
                data_set[i][3])  # the main categorie is now encoding with a number beetween 0 and 120
            input_data_set[i, 3] = list_of_categorie.index(
                data_set[i][2])  # the categorie is now encoding with a number
